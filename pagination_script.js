// next button
var btn_Next = document.querySelector('.btn-nxt')
btn_Next.addEventListener('click', ()=>{
    // event.preventDefault()
    console.log("Clicked")
    for(let i=1;i<=5;i++)
    {
        var taking_class=document.querySelector(`.btn-${i}`)
        var getting_attri=parseInt(taking_class.getAttribute('for'))

    if(getting_attri<=95){
        var calculation=getting_attri + 5
        taking_class.setAttribute('for',calculation)
        taking_class.innerText=calculation
       }
    }
})

// previous button
var btn_Previous = document.querySelector('.btn-pre')
...
//   To display data
function clicked_btn(event){
    document.querySelector('.all_data').innerHTML=''
       var get_for=parseInt(event.getAttribute('for'))-1

       console.log(get_for+1)
       var obtained_data = data[get_for]
       console.log(obtained_data)

    //    creating element
       var creating_div = document.createElement('div')
       creating_div.innerHTML=`
       <p for=${get_for+1}>${"Id :"}${obtained_data.id}</p>
       <p for="Name">${"Name :"} ${obtained_data.name}</p>
       <p for="Email">${"Email :"} ${obtained_data.email}</p>
       `
       document.querySelector('.all_data').append(creating_div)
